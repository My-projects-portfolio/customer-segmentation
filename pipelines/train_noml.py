...
def silhouette_numpy(X: np.ndarray, labels: np.ndarray, sample: int | None = None, seed: int = 42):
    """Compute silhouette score (mean) in NumPy. To keep it fast, you can sample points."""
    rng = np.random.default_rng(seed)
    n = X.shape[0]
    idx = np.arange(n) if (sample is None or sample >= n) else rng.choice(n, size=sample, replace=False)
    L = labels[idx]
...
def save_artifacts(export_path: str, centers: np.ndarray, mean: np.ndarray, std: np.ndarray, k: int, sil: float):
    out = Path(export_path)
    out.mkdir(parents=True, exist_ok=True)
    np.savez(out / "kmeans_model.npz", centers_=centers, scale_mean_=mean, scale_scale_=std)
    (out / "meta.json").write_text(json.dumps({"silhouette": sil, "k": int(k)}, indent=2))
...
def parse_args():
    ...
    ap.add_argument("--export_path", required=True, help="Where to write model artifacts")
    ap.add_argument("--sample", type=float, default=None, help="Optional frac sample (0-1) for speed")
...
def main():
    ...
    print(f"Trained (no-MLflow, no-sklearn) KMeans(k={args.k}) silhouette={sil:.4f}, n={len(Xs)}")
